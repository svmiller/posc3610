...
CINC <- read.csv("~/Dropbox/data/cow/nmc/NMC_5_0.csv") %>% tbl_df()
...
### MIC of the Day: Crimean War (MIC#0057)

![Franz Roubaud's (1904) Siege of Sevastopol](crimean-war.jpg)

### MIC of the Day: Crimean War (MIC#0057)

![An Excerpt from John Coleman's (1855) "The Four Points"](bop-crimean-war.png)
...
```{r cinc-scores-usa-ukg-gmy-rus-1816-2016, echo=F, eval=T, fig.width = 14, fig.height = 8.5}

NMC %>%
  filter(ccode %in% c(2, 200, 255, 260, 365)) %>%
  mutate(ccode = ifelse(ccode == 260, 255, ccode)) %>%
  mutate(state = countrycode::countrycode(ccode, "cown", "country.name")) %>%
  ggplot(.,aes(year, cinc, color=state, linetype = state)) +
  theme_steve_web() +
  geom_line(size = 1.5)  +
  scale_x_continuous(breaks=seq(1820, 2020, by=10)) +
  scale_color_brewer(palette="Set1") +
  labs(color = "", linetype = "", x = "", y = "CINC Score",
       title = "CINC Scores for the U.S., UK, Germany and Russia, 1816-2016",
       subtitle = "The U.S. has long been the most powerful country in the world, but notice the various power transitions implied by these data.",
       caption = "Source: Correlates of War National Military Capabilities Data (v. 6.0)")
